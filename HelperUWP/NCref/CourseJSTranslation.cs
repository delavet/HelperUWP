...
namespace HelperUWP.NCref
{
    class CourseJSTranslation
    {
        ...
        private static int[][] getKeyBytes(String key)
        {
            ...
            if (remainder > 0)
            {
                keyBytes[i] = strToBt(key.Substring(i * 4, leng-i*4));
            }
            return keyBytes;
        ...
        private static int[] sBoxPermute(int[] expandByte)
        {
            ...
            for (m = 0; m < 8; m++)
            {
                ...
                sBoxByte[m * 4] = Int32.Parse(binary.Substring(0, 1));
                sBoxByte[m * 4 + 1] = Int32.Parse(binary.Substring(1, 1));
                sBoxByte[m * 4 + 2] = Int32.Parse(binary.Substring(2, 1));
                sBoxByte[m * 4 + 3] = Int32.Parse(binary.Substring(3, 1));
            }
            return sBoxByte;
        }

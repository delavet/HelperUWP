...
namespace HelperUWP.HoleRef
{
    public sealed class WaterfallPanel : Panel
    {
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            KeyValuePair<double, int>[] flowLens = new KeyValuePair<double, int>[HoleInfoList.WaterCol];
            foreach (int idx in Enumerable.Range(0, HoleInfoList.WaterCol))
            {
                flowLens[idx] = new KeyValuePair<double, int>(0.0, idx);
            }

            // 我们就用2个纵向流来演示，获取每个流的宽度。
            double flowWidth = availableSize.Width / HoleInfoList.WaterCol;
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            double flowWidth = finalSize.Width / HoleInfoList.WaterCol;

            // 要用到流的横坐标了，我们用一个数组来记录（其实最初是想多加些花样，用数组来方便索引横向偏移。不过本例中就只进行简单的乘法了）
            double[] xs = new double[HoleInfoList.WaterCol];
